 ...
 * @since 2017-02-25
 */
public class DijkstraAlphaSolver extends DijkstraSolver
{
	private final MazeNode endNode;
	private final double alpha = 0.007;

	public DijkstraAlphaSolver(Maze maze, int interval)
	{
		super(maze, interval);
		endNode = maze.getEnd();
		setComparator(Comparator.comparingDouble(this::getScore));
	}

	private double getScore(MazeNode node)
	{
		return alpha * node.getDistance() + (1 - alpha) * getFlightDistance(node);
	}
